...
| `[search]` `population` | `"auto"` | What the search cache holds for issues/MRs: `"tracked"` (what `"auto"` resolves to) = lazy — live search lookups feed the cache and the background sync refreshes only projects with recent evidence of relevance (assigned items, history, live hits in member projects); `"all"` = everything the token can see (`scope=all`; infeasible on large instances); `"member"` = every member project (two fetches per project). Projects and groups are always membership-scoped. |
