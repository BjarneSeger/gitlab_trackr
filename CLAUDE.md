...
- **Refresh** (`handlers/refresh.rs`): startup warm-up (issues/boards first, then history backfill — order matters, history enrichment reads project IDs from the issue cache), a quick tier (default 5 min: issues, boards, recent timelog window) and a slow tier (daily: bulk history + retention pruning). Intervals are re-read each tick so the hot config reload (`reload.rs`) applies without restart.
